...
function handleMove(ws, userId, matchId, move, signature) {
  ...
  // Notify opponent with enhanced move data
  ...
  if (opponentConnection) {
    opponentConnection.send(JSON.stringify({
      type: 'opponentMove',
      matchId,
      moveIndex: match.moveHistory.length - 1,
      move: {
        from: move.from,
        to: move.to,
        piece: move.piece,
    ...
    console.warn(`Opponent ${opponentId} not connected, move notification not sent`);
  }
  
  // Send confirmation to player with move details
  ws.send(JSON.stringify({
    type: 'moveConfirmed',
    matchId,
    moveIndex: match.moveHistory.length - 1,
  ...
  // If game over, notify both players
  if (gameOver) {
    ...
    // Save the completed match to database
    saveMatchToDatabase(match);
    console.log(`Match ${matchId} ended by ${match.result.reason}. Winner: ${match.result.winner}`);
  }
}
