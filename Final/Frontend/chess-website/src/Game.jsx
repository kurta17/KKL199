    ...
    try {
      ...
      if (privateKey) {
        try {
          ...
          signature = await wallet.signMessage(moveDataString);
          console.log('Move signed successfully:', signature.substring(0, 10) + '...');
        } catch (signingError) {
          console.error('Error signing move:', signingError);
          setErrorMessage('Error signing move. Continuing without blockchain verification.');
        }
      } else {
        console.warn('Private key not found. Move will be sent without signature.');
        setErrorMessage('Private key not found. Move will be sent without blockchain verification.');
      }
