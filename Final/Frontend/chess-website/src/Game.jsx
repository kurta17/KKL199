...
function Game() {
  ...
  const handleGameState = (data) => {
    ...
    if (data.opponent) {
      ...
      setOpponent({
        id: data.opponent.id,
        username: data.opponent.username || `Player ${data.opponent.id?.substring(0, 8) || 'Unknown'}`,
  ...
  // Helper function to set up socket handlers
  const setupSocketHandlers = (socket) => {
    ...
    socket.onMove(handleOpponentMove);
    socket.onGameOver(handleGameOver);
    socket.onDrawOffer(handleDrawOffer);
    socket.onGameState(handleGameState);
  ...
  const connectWebSocket = () => {
    ...
    debugMsg.textContent = `Attempting connection at ${new Date().toLocaleTimeString()}`;
    debugMsg.style.color = 'yellow';
    const debugElem = document.getElementById('lastSocketEvent');
    if (debugElem) {
      debugElem.appendChild(debugMsg);
    }
  ...
  const retryConnection = () => {
    ...
    // Increment reconnect attempts (only once)
    setReconnectAttempts(prev => prev + 1);
  };
  ...
  useEffect(() => {
    ...
    // Force a fresh render of the chess board
    setFen(chess.fen());
  ...
  const handleMove = async (move) => {
    ...
    try {
      ...
      // Sign the move with Ed25519 private key
      const privateKeyBase64 = localStorage.getItem('userPrivateKey');
      let signature = null;
      
      if (privateKeyBase64) {
        try {
          // Convert the base64 private key to Uint8Array
          const privateKeyBytes = base64js.toByteArray(privateKeyBase64);
          
          // Create the message to sign
          const moveDataString = JSON.stringify(moveData);
          const messageBytes = new TextEncoder().encode(moveDataString);
          
          // Sign the message with Ed25519
          const signatureBytes = nacl.sign.detached(messageBytes, privateKeyBytes);
          
          // Convert signature to base64 for transmission
          signature = base64js.fromByteArray(signatureBytes);
          
          console.log('Move signed successfully with Ed25519:', signature.substring(0, 10) + '...');
        } catch (signingError) {
          console.error('Error signing move with Ed25519:', signingError);
          setErrorMessage('Error signing move. Continuing without blockchain verification.');
        }
      } else {
        console.warn('Ed25519 private key not found. Move will be sent without signature.');
        setErrorMessage('Ed25519 private key not found. Move will be sent without blockchain verification.');
      }

      // Enhance move object with extra data needed by the server
      const enhancedMove = {
        ...move,
        fen: newFen,
        piece: piece,
        promotion: promotion,
        gameState: getGameState(chess),
        capture: result.captured ? true : false,
        check: chess.inCheck()
      };
      ...
      if (socketRef.current) {
        ...
        socketRef.current.sendMove(gameId, enhancedMove, signature);
      }
  ...
  const getGameState = (chessInstance) => {
    ...
    if (chessInstance.isInsufficientMaterial()) return 'insufficient';
    return null;
  };
  ...
  // Render debug panel for WebSocket connections
  const renderDebugPanel = () => {
    // Always show the debug panel to help diagnose connection issues
    return (
      <div style={{
        position: 'fixed',
        bottom: 10,
        ...
        <p>Status: <span style={{color: 
          connectionStatus === 'connected' ? 'lightgreen' : 
          connectionStatus === 'connecting' ? 'yellow' : 'red'
        }}>{connectionStatus}</span></p>
        <p>WebSocket URL: {socketRef.current?.currentUrl || 'ws://localhost:5000'}</p>
        <p>Connection attempts: {reconnectAttempts}</p>
        <p>WebSocket State: {socketRef.current?.socket ? 
          ['Connecting', 'Open', 'Closing', 'Closed'][socketRef.current?.socket?.readyState] : 'None'}</p>
        <div style={{display: 'flex', gap: '10px'}}>
          ...
          }} style={{background: '#444', border: '1px solid #888', padding: '5px 10px', 
                    cursor: 'pointer', borderRadius: '3px', flex: 1}}>
            Reconnect
          </button>
          <button onClick={retryConnection} style={{
            background: '#554', border: '1px solid #888', padding: '5px 10px', 
            cursor: 'pointer', borderRadius: '3px', flex: 1
          }}>
            Try Alternative
          </button>
        </div>
        <p>Browser: {navigator.userAgent.substring(0, 80) + '...'}</p>
        <button onClick={() => window.location.reload()} style={{
          background: '#733', border: '1px solid #888', padding: '5px 10px',
          cursor: 'pointer', borderRadius: '3px', width: '100%', marginTop: '10px'
        }}>
          Reload Page
        </button>
      </div>
  ...
  return (
    <div className="min-vh-100 text-white d-flex flex-column p-4 position-relative">
      ...
      <div className="container position-relative mt-5" style={{zIndex: "10", maxWidth: "900px"}}>
        ...
        {!isMatched ? (
          <div className="glass-card p-4 text-center">
            ...
            {connectionStatus !== 'connected' && (
              <div className="mt-3">
                <div className={`alert alert-${connectionStatus === 'connecting' ? 'warning' : 'danger'}`}>
                  {connectionStatus === 'connecting' ? 
                    'Connecting to game server...' : 
                    'Cannot connect to game server. Please check your connection and refresh the page.'}
                </div>
                {connectionStatus === 'disconnected' && (
                  <button className="btn btn-primary mt-2" onClick={() => connectWebSocket()}>
                    Retry Connection
                  </button>
                )}
              </div>
          ...
          <div className="glass-card p-4 text-center">
            <p className="text-light mb-3">
              You have been matched with <strong>{opponent?.username || `Player ${opponent?.id?.substring(0, 8) || 'Unknown'}`}</strong>
          ...
          <div className="row g-4">
            ...
            <div className="col-md-7">
              <div className="glass-card p-3">
                ...
                <div className="mb-2 p-2 bg-dark bg-opacity-75 text-center rounded">
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <div>
                      {opponent ? (
                        <>
                          <strong className="text-light">{opponent.username || `Player ${opponent.id?.substring(0, 8) || 'Unknown'}`}</strong>
                          {opponent.rating && <span className="text-warning ms-2">Rating: {opponent.rating}</span>}
                        </>
                      ) : (
                        <strong className="text-light">Waiting for opponent...</strong>
                      )}
                    </div>
                    <div>
                      {opponent && opponent.wins !== undefined && opponent.losses !== undefined && (
                ...
                <div className="board-container" key={`board-container-${color}`} style={{
                  margin: '0 auto',
                  maxWidth: '100%',
                  ...
                  <Chessboard
                    ...
                    boardWidth={window.innerWidth < 576 ? 300 : 400}
                    ...
                    customBoardStyle={{
                      ...
                      display: 'block', // Ensure the board is visible
                      visibility: 'visible',
                      opacity: 1,
                      background: '#8A8A8A'
                    }}
            ...
            <div className="col-md-5">
              <div className="glass-card p-3 h-100">
                ...
                <div className="mb-3">
                  ...
                  <p className="mb-1"><strong>Opponent:</strong> {opponent ? (opponent.username || `Player ${opponent.id?.substring(0, 8) || 'Unknown'}`) : 'Waiting for opponent...'}</p>
                </div>
        ...
        {/* Debug panel for WebSocket connections - always show to help diagnose the issue */}
        {renderDebugPanel()}
      </div>
    </div>
  );
