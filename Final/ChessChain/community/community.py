...
class ChessCommunity(Community):
    ...
    def get_stored_moves(self, match_id: str) -> List[MoveData]:
        ...
        Returns:
            List of MoveData objects for the specified match, sorted by move number
        """
        out = []
        ...
        with self.db_env.begin(db=self.moves_db) as txn:
            ...
            # First try with the new composite key format: match_id:move_id
        ...
        # Sort moves by move number to ensure correct ordering
        return sorted(out, key=lambda x: x.move_number)
    ...
    async def broadcast_move(self, move: MoveData) -> None:
        """Broadcast a move to all peers in the network.
        
        Args:
            move: The move data to broadcast
        """
        self.logger.info(f"Broadcasting move: {move.id} for match {move.match_id}")
        ...
        try:
            ...
            with self.db_env.begin(write=True) as txn:
                # Use a composite key: match_id:move_id
                move_key = f"{move.match_id}:{move.id}".encode('utf-8')
                txn.put(move_key, packed_move, db=self.moves_db)
            
            self.logger.info(f"Successfully stored move {move.id} in local database")
        except Exception as e:
            self.logger.error(f"Error storing move in database: {e}")
            
        # Broadcast to all peers
        try:
            for peer in self.get_peers():
                self.ez_send(peer, move)
                self.logger.info(f"Sent move {move.id} to peer {peer.mid.hex()[:8]}")
        except Exception as e:
            self.logger.error(f"Error broadcasting move to peers: {e}")
